...
const getTechByBuilding = async (req, res) => {
    ...
    building = replaceChar("-", " ", building);
    building = await Building.find({ name: building });
    console.log(building[0].id);

    let tech = await Tech.find();
    tech = tech.filter(x => x.research.at == building[0].id);
    tech = tech.map(x => x.name);

    if (tech) {
        res.json({
            building: building[0].name,
            techs_in_building: tech
        });
    } else {
        res.status(404).json({
            status: 404,
